...
import { PORT, FRONTEND_ORIGIN } from './config/env.mjs';
import {
  strategicClients,
  ansMatrix,
  metricasDefiniciones
} from './lib/rag.mjs';
import { classifyTicket } from './services/classifier.mjs';
...
app.get('/api/clients', (_req, res) => {
  // Normalizar clientes para el frontend
...
app.post('/api/tickets', async (req, res) => {
  ...
  try {
    ...
    // Si ansSla tiene los campos directamente (del servicio ANS), usarlos
    const slaObjetivo = {
...
app.listen(PORT, () => {
  console.log(`Ticket Triage Copilot API escuchando en http://localhost:${PORT}`);
});
